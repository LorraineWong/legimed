...
DRUG_KEYWORDS = {"drug", "drugs", "medication", "medications", "medicine", "medicines",
                 "maoi", "inhibitor", "inhibitors", "nsaid", "nsaids", "antibiotic",
                 "antibiotics", "supplement", "supplements"}


def _is_food(substance: str) -> bool:
    """Return True only if substance looks like a food/drink, not a drug."""
...
def format_html_output(drug_info, personal_summary) -> str:
    ...
    def section(label, content):
        ...
        <div style="background:#fff;border-radius:16px;
          ...
          <div style="font-size:10px;font-weight:700;color:#00A878;text-transform:uppercase;
    ...
    <div style="font-family:system-ui,-apple-system,'Segoe UI',sans-serif;
                background:#F7FAFC;padding:14px;border-radius:18px;max-width:560px;margin:0 auto;">

      <div style="background:linear-gradient(135deg,#00A878,#00875F);border-radius:16px;
                  padding:18px;margin-bottom:10px;color:#fff;">
        <div style="font-size:22px;font-weight:800;line-height:1.2;">{drug_info.drug_name}</div>
        <div style="font-size:12px;opacity:0.9;margin-top:5px;">
          {drug_info.active_ingredient}</div>
        <div style="display:inline-block;margin-top:8px;background:rgba(255,255,255,0.2);
                    border:1px solid rgba(255,255,255,0.3);padding:4px 10px;border-radius:999px;
                    font-size:11px;font-weight:600;">{drug_info.drug_class}</div>
      </div>"""
    ...
    for slot, icon in time_slots.items():
        ...
        if d:
            slots_html += f"""
            <div style="background:#E6FFFA;border:1px solid #8FDCC5;border-radius:10px;
            ...
            slots_html += f"""
            <div style="background:#EDF2F7;border-radius:10px;padding:8px 4px;text-align:center;">
              <div style="font-size:16px;opacity:0.25;">{icon}</div>
    ...
    if drug_info.side_effects:
        se_html = ""
        for se in drug_info.side_effects[:4]:
            ...
            <div style="display:flex;align-items:flex-start;gap:8px;padding:9px 10px;
                        border-radius:10px;border-left:3px solid {border};
              ...
              <div style="font-size:9px;padding:3px 7px;border-radius:999px;
                          background:{tag_bg};color:{tc};font-weight:700;white-space:nowrap;">
                {tag}</div>
            </div>"""
        html += section("⚡ Side Effects", se_html)

    # Food & drink — filter out drugs
    ...
    if drug_info.warnings:
        ...
        for w in drug_info.warnings[:3]:
            ...
            <div style="font-size:12px;color:#744210;padding:4px 0;
                        border-bottom:1px solid #FDE68A;line-height:1.6;">• {w.text}</div>"""
        html += f"""
        <div style="background:#FFFBF0;border-left:3px solid #F6AD55;border-radius:16px;
                    box-shadow:0 2px 8px rgba(0,0,0,0.06);padding:16px;margin-bottom:10px;">
          <div style="font-size:10px;font-weight:700;color:#B7791F;text-transform:uppercase;
                      letter-spacing:0.08em;margin-bottom:8px;">⚠️ Warnings</div>
          {w_html}</div>"""
    ...
    if drug_info.emergency_signs:
        ...
        for e in drug_info.emergency_signs[:3]:
            e_html += f"""
            <div style="font-size:12px;color:#7F1D1D;padding:3px 0;line-height:1.6;">• {e}</div>"""
        html += f"""
        <div style="background:#FFF5F5;border:1px solid #FEB2B2;border-radius:16px;
                    box-shadow:0 2px 8px rgba(0,0,0,0.06);padding:16px;margin-bottom:10px;">
          <div style="font-size:10px;font-weight:800;color:#C53030;text-transform:uppercase;
                      letter-spacing:0.08em;margin-bottom:8px;">🚨 Seek Help Immediately If:</div>
          {e_html}</div>"""
    ...
    try:
        ...
        if drug_name:
            ...
            <div style='background:#F0FDF4;border:1px solid #6EE7B7;border-radius:10px;
              ...
              <span style='font-size:11px;color:#718096;margin-left:6px;'>(via {method})</span><br>
...
def build_demo(model, tokenizer, processor=None):
    ...
    with gr.Blocks(
        ...
        <div style="text-align:center;padding:20px 0 10px;">
          ...
          <div style="font-size:28px;font-weight:800;color:#1A202C;margin-top:2px;">Legimed</div>
          <div style="font-size:13px;color:#718096;margin-top:4px;">
            Your medication, made legible</div>
        </div>""")

        gr.HTML("""<div style="display:flex;align-items:center;gap:8px;margin:12px 0 8px;">
          ...
          <div style="font-size:13px;font-weight:700;color:#1A202C;">
            Scan medicine box or type drug name</div>
        </div>""")

        with gr.Group():
            image_input = gr.Image(
                type="pil",
                label="📷 Camera / Upload (optional)",
        ...
        gr.HTML("""<div style="display:flex;align-items:center;gap:8px;margin:16px 0 8px;">
          <div style="width:22px;height:22px;border-radius:50%;background:#00A878;color:#fff;
                      font-size:11px;font-weight:800;display:flex;align-items:center;
        ...
        with gr.Group():
            age_input  = gr.Radio(
                choices=["adult", "elderly"], value="adult", label="Age group")
        ...
        generate_btn.click(
            fn=lambda: """<div style='text-align:center;padding:2rem 1rem;color:#00A878;
